...
export const GraphNode: React.FC<GraphNodeProps> = ({ node, onClick, onRemove, style, animationDelay }) => {
  ...
  return (
    ...
    >
      <div
        className={
          `relative w-24 h-24 bg-slate-800 rounded-full flex flex-col items-center justify-center
      ...
      <h3 className="mt-2 text-sm font-bold text-white transition-colors duration-300 group-hover:text-cyan-400 max-w-[120px] truncate" title={node.name}>
        {node.name}
      </h3>
    </div>
  );
